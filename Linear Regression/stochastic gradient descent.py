...
def gradient_descent(data, w, b, lr):
        ...
        for step in range(max_steps):
                ...
                random_data_sample = [data[random_i]]

                # Calc gradient
                gradient = [0] * fsize
                for j in range(fsize):
                        for x in random_data_sample:
                                y = x[-1]
                                wx = dot(w, x)
                ...
                for x in data:
                        y = x[-1]
                        wx = dot(w, x)
                        cost += decimal.Decimal(y - wx - b) ** 2
                cost *= decimal.Decimal(1/2)
                

                # save to csv file for graphing
                #f = open("stochastic_cost_func_values.csv", "a")
                #f.write(str(cost) + "\n")
                #f.close()
...
def main():
        ...
        w = [0] * fsize
        b = 0
        lr = 0.003125 # learning rate

        best_weight = gradient_descent(data, w, b, lr)
        #best_weight = [0.026, -0.213, -0.168, 0.373, -0.044, 0.083, -0.001]
        print(best_weight)
