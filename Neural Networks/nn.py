...
def forward_pass(x, weights, num_of_layers, nn_width, input_width, question):
    ...
    for layer in range(1, num_of_layers):
        ...
        for node_index in range(input_width - 1):
            prev_layer = layer - 1
...
def nn(data, w, lr, t, a, nn_width, input_width, question):
    ...
    for epoch in range(t):
        ...
        prev_w = list(x[weight_value] for x in w)
        ...
        for x in copy.deepcopy(data): # copy is important, otherwise we're changing the actual data (fixed bug)
            ...
            # update weight with it
            weight_values = list(x[weight_value] for x in w)
            updated_weight_values = sub(weight_values, scale(gradient, r))
            for i in range(len(w)):
                w[i][weight_value] = updated_weight_values[i]
            
            #print(w)

        # Stop iterating when the length of the weight difference vector
        # (from the prev iteration) is less than the tolerance level
        w_diff = sub(prev_w, list(x[weight_value] for x in w))
        tolerance = 10e-3
...
def print_errors(learned_weight, data, test_data, num_of_layers, nn_width, input_width, question):
    # ------------------------
    ...
    for x in copy.deepcopy(data):
        ...
        # find our prediction
        _, y = forward_pass(x, learned_weight, num_of_layers, nn_width, input_width, question)
